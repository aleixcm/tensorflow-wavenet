...
def genSignals(sequence, sampleSequence, c):
    ...
    for i in range(len(sequence)):
        ...
        #a = 1
        ysin = a*np.sin(2*np.pi*freq*nSamples / fs)
        ysquare = a*signal.square(2*np.pi*freq*nSamples / fs)
        ysawtooth = a*signal.sawtooth(2*np.pi*freq*nSamples / fs)
    ...
    scipy.io.wavfile.write(os.path.join('corpus', 'localTrainBigDatasetShapeAmp', 'lc_train%s.wav' % c), fs, y)
